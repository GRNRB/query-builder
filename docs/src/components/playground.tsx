...
// Live, embeddable version of the interactive demo.
//
// Source of truth: examples/basic/src/App.tsx. This is a trimmed port — the same widget
// (schema, drag state, GroupBlock/ConditionRow/DropZone, uncontrolled demo) without the
// standalone app chrome (page <h1>, full-viewport wrapper, controlled/uncontrolled tabs).
// If the example's component shape changes, re-sync this file. It imports the library via
// `@grnrb/react-query-builder` (workspace:*), so it always exercises the local source.
...
/**
 * Embeddable live demo. Mounts client-side only: the initial tree is normalized with
 * `crypto.randomUUID()` (non-deterministic), which would otherwise cause a hydration
 * mismatch against the statically pre-rendered HTML. The placeholder keeps layout stable.
 */
